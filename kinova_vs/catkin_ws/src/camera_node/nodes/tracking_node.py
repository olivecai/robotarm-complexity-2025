...
class TrackingNode:
    ...
    def callback(self, imgmsg):
        ...
        if self.first_frame == 0: #beginning of the visual servoing
            ...
            self.old_points = np.array([np.array(points[0]), np.array(points[1])])

            self.old_gray = cv2.cvtColor(rgb_frame, cv2.COLOR_RGB2GRAY)

            self.first_frame = 1 #the first frame has been taken in
            ...
            self.new_points = np.array([np.array(new_points[0]), np.array(new_points[1])])
