...
May 7 2025
Added functionality in "plot_error" to plot error at each q0 q1 pair.
'''
import numpy as np
...
def inverse_kinematics_2d(e, theta, desired_pos, tolerance, maxiter, J): #uses a constant Jacobian. So, do this step for every q0 q1 in our meshgrid?
    ...
    while iter_var < maxiter and error > tolerance:
        ...
        error = norm(residual)
        theta = old_theta + correction_step*0.5 #NOTE: we can mult corr step by damping

    xerror = residual[0]
    yerror = residual[1]
    return error, xerror, yerror

def plot_error(e, desired, tolerance, maxiter, J, mode):
    '''
    ...
    for i in range(n):
        for j in range(n):
            ...
            if mode==2: 
                position= get_pos(e, np.array([q0,q1]))
                TotalError[i,j]=norm(position-desired)
                XError[i,j]=((position-desired)[0])
                YError[i,j]=((position-desired)[1])
    ...
    if mode==1:
        plt.suptitle("Mode 1: Measure error after ikine using Constant Jacobian from q0,q1")
    if mode==2:
...
def main():
    ...
    plot_error(e2, DESIRED, tolerance, maxiter, J, MODE)
