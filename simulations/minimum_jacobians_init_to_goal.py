...
solve for milestones from init to goal
...
class Trajectory():
    def __init__(self, ets: rtb.ETS, joint_ranges : list, joint_ranges_full: list, convergence_algorithm_params : dict = None): 
        ...
        # DELIVERABLES
        self.closest_success_point_to_goal_dist = 0 #when doing init -> goal
        self.closest_success_point_to_starting_position_dist = 0 #when doing goal -> init
...
def get_convergence_region_forwards(Trajectory: Trajectory):
    '''
    for each Q in the randomly sampled joint configurations,
    attempt the inverse kinematics to get to each cartesian position from fkin(Q).
    
    Next, consider the nD space of the cartesian world plus the n-3 joint space of the base(s) joints of the robot.
    (ie if we choose the first two joints of the robot, we have a 5D space.)

    Compare this space to 
    '''


def get_convergence_region_backwards(Trajectory: Trajectory):
    '''
    CARTESIAN SPACE convergence region.
